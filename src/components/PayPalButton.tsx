...
export default function PayPalButton({ 
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        onErrorRef.current?.(e?.message || 'Failed to load PayPal SDK');
      }
    })();
  ...
  useEffect(() => {
    ...
    const buttons = paypal.Buttons({
      ...
      createOrder: (_data: any, actions: any) => {
        ...
        const desc = descriptionRef.current || 'Payment';
      ...
      onApprove: async (_data: any, actions: any) => {
        try {
          ...
          onErrorRef.current?.(e?.message || 'Payment capture failed');
        }
      },
      onCancel: () => { 
    ...
    if (typeof buttons.isEligible === 'function' && !buttons.isEligible()) {
      console.warn('[paypal] Buttons not eligible for this context', { 
        amount: amountRef.current, 
        currency: currencyRef.current 
      });
      onErrorRef.current?.('PayPal buttons not eligible for this payment');
      return;
    }
    ...
    buttons
      ...
      .then(() => { 
        console.log('[paypal] render success'); 
        setRendered(true); 
      })
      .catch((err: any) => {
        console.error('[paypal] render failed', err);
        onErrorRef.current?.(err?.message || 'Failed to render PayPal buttons');
      });
    ...
    return () => {
      ...
      if (!stillInDom && buttonsRef.current) {
        try { 
          ...
          console.log('[paypal] buttons closed (unmount)'); 
        } catch {}
  ...
  return (
    <div className="flex flex-col items-center gap-2 w-full max-w-sm">
      {!ready && (
        <Button disabled size="default" className="w-full">
